...
@Component
@Slf4j
@ServerEndpoint("/ws/community/{communityId}/{userId}")
public class WeCommunityServer {

    //全局静态唯一Map：存储所有连接实例（key: communityId:userId）
    private static final ConcurrentHashMap<String, WeCommunityServer> ONLINE_USER_MAP = new ConcurrentHashMap<>();
    //全局静态唯一Map：存储每个圈子的在线用户ID集合（key: communityId）
    private static final ConcurrentHashMap<Integer, Set<Integer>> COMMUNITY_ONLINE_USER_IDS_MAP = new ConcurrentHashMap<>();

    //每个实例独有的属性
    private Session session;
    private Integer communityId;  // 圈子ID（从路径参数获取）
    private Integer userId;    // 用户ID（从路径参数获取）
    private String uniqueKey;  // 连接唯一标识：communityId:userId
    ...
    public void setWeCommunityService(WeCommunityService weCommunityService) {
        WeCommunityServer.weCommunityService = weCommunityService;
    }
     ...
     */
    @OnOpen
    public void onOpen(Session session, @PathParam("communityId") Integer communityId, @PathParam("userId") Integer userId) {
        this.session = session;
        this.communityId = communityId;
        this.userId = userId;
        this.uniqueKey = communityId + ":" + userId;

        log.info("用户[{}]尝试连接圈子[{}]，连接标识：{}", userId, communityId, uniqueKey);

        // 加入在线用户Map和圈子在线列表
        ONLINE_USER_MAP.put(uniqueKey, this);
        // 先根据圈子ID，查有没有对应的在线用户集合
        Set<Integer> onlineUserIds = COMMUNITY_ONLINE_USER_IDS_MAP.get(communityId);
        //如果没有这个集合（说明这个圈子第一次有人上线）
        if (onlineUserIds == null) {
            onlineUserIds = ConcurrentHashMap.newKeySet();
            // 把“圈子ID-在线集合”存入 Map
            COMMUNITY_ONLINE_USER_IDS_MAP.put(communityId, onlineUserIds);
        }
        //把当前用户ID加入在线集合
        onlineUserIds.add(userId);

        log.info("用户[{}]连接圈子[{}]成功！当前圈子在线人数：{}",
                userId, communityId, COMMUNITY_ONLINE_USER_IDS_MAP.get(communityId).size());
    ...
    public void onMessage(String message) {
        try {
            ...
            //广播消息给圈子所有成员（在线+离线）
            broadcastMessage(chatInfoEntity);
    ...
    @OnClose
    public void onClose() {
        log.info("用户[{}]断开圈子[{}]连接", userId, communityId);
        ...
        if (onlineUserIds != null) {
            ...
            if (onlineUserIds.isEmpty()) {
                COMMUNITY_ONLINE_USER_IDS_MAP.remove(communityId);
                log.info("圈子[{}]无在线用户，清除缓存", communityId);
            }
        }
    ...
    public void onError(Throwable throwable) {
        log.error("用户[{}]圈子[{}]连接异常", userId, communityId, throwable);
     ...
     * 广播消息给圈子所有成员（在线+离线）
     */
    private void broadcastMessage(ChatInfoEntity chatInfoEntity) {
        try {
            ...
            if (allMemberIds == null || allMemberIds.isEmpty()) {
                log.info("圈子[{}]无成员，无需广播", communityId);
                return;
            }

            // 2. 获取当前圈子在线用户列表
            Set<Integer> onlineUserIds = COMMUNITY_ONLINE_USER_IDS_MAP.getOrDefault(communityId, ConcurrentHashMap.newKeySet());
            ...
            for (Integer memberId : allMemberIds) {
                if (onlineUserIds.contains(memberId)) {
                    ...
                    WeCommunityServer onlineController = ONLINE_USER_MAP.get(onlineUniqueKey);
                    ...
                    if (onlineController != null) {
                        ...
                        // 异常：在线用户无连接，按离线处理
                        handleOfflineMember(chatInfoEntity, memberId);
                    }
                } else {
                    // 离线成员：创建未读关联记录
                    handleOfflineMember(chatInfoEntity, memberId);
     ...
     * 处理离线成员：创建未读关联记录
     */
    private void handleOfflineMember(ChatInfoEntity chatInfoEntity, Integer memberId) {
        try {
            ChatInfoRelaEntity connect = new ChatInfoRelaEntity();
            connect.setUserId(memberId);
            connect.setMsgId(chatInfoEntity.getMsgId());
    ...
    private void pushOfflineMessages() {
        try {
            ...
            if (offlineMessages == null || offlineMessages.isEmpty()) {
                log.info("用户[{}]圈子[{}]无离线消息", userId, communityId);
                return;
            }
